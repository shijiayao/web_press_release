<template>
  <section class="notice-manage-wrap">
    <section class="head-box">
      ...
      <el-button type="success" @click="headAddButton">新增公告信息</el-button>
    </section>
    ...
    <section class="table-box">
      <el-table size="default" :data="tableData" border style="width: 100%" empty-text="没有数据" @cell-click="tableCellClick">
        ...
        <el-table-column header-align="center" align="center" prop="level" label="用户组" width="90">
      ...
      <el-form label-width="80px" :model="tableRowData">
        <el-form-item label="用户组:">
          <el-select v-model="tableRowData.level">
            ...
            <el-option label="所有用户" :value="2000"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="公告标题:">
          <el-input v-model="tableRowData.title" placeholder="请输入公告信息标题"></el-input>
        </el-form-item>
    ...
    <el-dialog v-model="detailDialogVisible" :destroy-on-close="true" custom-class="show-dialog">
      ...
      <section class="detail-wrap">
        <article class="detail-content" v-html="showDetailContent"></article>
      </section>
      <template #footer>
        <span class="dialog-footer">
...
export default {
  ...
  computed: {
    showDetailContent() {
      return this.tableRowDetailData.content
        .split('\n')
  ...
  methods: {
    ...
    // 头部新增按钮
    headAddButton() {
      this.tableRowData = { level: 2000, title: '', content: '' };
      this.dialogMode = 'add';
    ...
    addDialogButton() {
      const _this = this;
      const { level, title, content } = _this.tableRowData;

      if (!level) {
        ElMessage.error('请选择公告所在的用户组，这将决定哪些用户可以看到公告！');

        return;
      }

      if (!title) {
        ElMessage.error('公告信息标题不能为空');

        return;
      }

      if (!content) {
        ElMessage.error('公告信息内容不能为空');
    ...
    editDialogButton() {
      const _this = this;
      const { index, id, level, title, content } = _this.tableRowData;
      let postData = {
        op_type: 1,
        id: id
      };
      ...
      if (!level) {
        ...
        return;
      } else if (level !== _this.tableData[index].level) {
